...
# =============================================================================
# OPTIMIZED BOOKVERSE CLEANUP SCRIPT  
# =============================================================================
# Optimized version with helper functions to reduce code duplication
...
# Optimized application version deletion
    ...
    if [[ "$code_versions" -ge 200 && "$code_versions" -lt 300 ]] && [[ -s "$versions_file" ]]; then
        ...
        if [[ ${#versions[@]} -gt 0 ]]; then
            ...
            for ver in "${versions[@]}"; do
                ...
                handle_deletion_response "$del_ver_code" "$ver" "VERSION"
            done
        else
...
# =============================================================================
# AUTHENTICATION SETUP (kept as-is)
# =============================================================================
...
# Generic API call with consistent error handling (kept as-is but optimized logging)
make_api_call() {
...
# Apply jq filter based on resource type and filter specification (kept as-is - already optimized)
apply_filter() {
...
# Enhanced discovery function (kept as-is - already optimized)
discover_resource() {
    local resource_type="$1"
    ...
    if is_success "$code" && [[ -s "$response_file" ]]; then
        ...
        case "$resource_type" in
            "repositories") echo "Found $count repositories with '$PROJECT_KEY' prefix" >&2 ;;
            "users") echo "Found $count users with '@bookverse.com' domain" >&2 ;;
            "applications") echo "Found $count applications in project '$PROJECT_KEY'" >&2 ;;
            "stages") echo "Found $count project stages with '$PROJECT_KEY-' prefix" >&2 ;;
            "lifecycle") echo "Found lifecycle configuration with promote stages: $(cat "$items_file")" >&2 ;;
            "project") echo "Project '$PROJECT_KEY' exists" >&2 ;;
        esac
...
# =============================================================================
# OPTIMIZED DELETION FUNCTION
# =============================================================================

# Generic deletion function for all resource types (OPTIMIZED)
delete_resource() {
    ...
    case "$resource_type" in
        ...
        "project")
            ...
            if is_success "$code"; then
                ...
                if is_not_found "$verify_code"; then
                ...
                while IFS= read -r item; do
                    if [[ -n "$item" ]]; then
                        ...
                        if [[ "$resource_type" == "applications" ]]; then
                            delete_application_versions "$item" "$TEMP_DIR"
                        fi
                        ...
                        if handle_deletion_response "$code" "$item" "$(echo "$display_name" | sed 's/s$//' | sed 's/repositorie/repository/' | tr '[:lower:]' '[:upper:]')"; then
                            ((deleted_count++))
                            ...
                            if [[ "$resource_type" == "repositories" ]]; then
                                ...
                                if is_not_found "$verify_code" || [[ "$verify_code" -eq $HTTP_BAD_REQUEST ]]; then
...
# Delete all versions for each application (OPTIMIZED - reuses delete_application_versions)
delete_all_application_versions() {
    ...
    while IFS= read -r app_key; do
        [[ -z "$app_key" ]] && continue
        delete_application_versions "$app_key" "$TEMP_DIR"
    done < "$apps_file"
}

# Delete all artifacts from each project repository (kept as-is)
